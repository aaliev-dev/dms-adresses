...
CLINIC_START = re.compile(
    r"^(ФГБУЗ|ФГАОУ|ФГБНУ|ФГБОУ|ГБУЗ|ГАУЗ|ФГБУ|ООО|МУЗ|ГКУ|ЗАО|ФБУЗ|ФБУ|ЧУЗ|ИММА|ФГАУ|АО|ПК|ГКБ|Больница|Медицинское|Частное|Общество|Филиал)(?=[\s«\":]|$)"
)
...
SERVICE_START = re.compile(r"^(Амбулаторно|Медицинская помощь|Стоматологическая помощь|Экстренная|Запись на приём|-\s*Запись)")
...
SERVICE_ANY = re.compile(r"(?:^|\s)(Амбулаторно|Медицинская помощь|Стоматологическая помощь|Экстренная)")
