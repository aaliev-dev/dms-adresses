...
CLINIC_TOKENS = (
    "ФГБУЗ", "ФГАОУ", "ФГБНУ", "ФГБОУ", "ГБУЗ", "ФГБУ", "ООО", "МУЗ",
    "ГКУ", "ЗАО", "ФБУЗ", "ФБУ", "ЧУЗ", "ИММА", "ФГАУ", "АО",
    "Медицинское", "Частное", "Общество", "Филиал",
)
CLINIC_START = re.compile(
    r"^(ФГБУЗ|ФГАОУ|ФГБНУ|ФГБОУ|ГБУЗ|ФГБУ|ООО|МУЗ|ГКУ|ЗАО|ФБУЗ|ФБУ|ЧУЗ|ИММА|ФГАУ|АО|Медицинское|Частное|Общество|Филиал)(?=[\s«\":])"
)
...
SERVICE_START = re.compile(r"^(Амбулаторно|Медицинская помощь|Стоматологическая|Запись на приём|-\s*Запись)")
# inline-граница услуги — только если услуга реально приклеена к клинике/телефону
SERVICE_ANY = re.compile(r"(?:^|\s)(Амбулаторно|Медицинская помощь|Стоматологическая)")
...
def main() -> int:
    ...
    for idx, e in enumerate(entries, start=1):
        address = normalize(" ".join(e["address"]))
